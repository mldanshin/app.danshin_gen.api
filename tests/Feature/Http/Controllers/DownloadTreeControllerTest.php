...
final class DownloadTreeControllerTest extends TestCase
{
    ...
    public function test200(string $personId, string $request, string $expectedFileName): void
    {
        ...
        $response = $this->get(route('download.tree', ['id' => $personId]).$request, $this->getHeaderRelativeSsoToken());
    ...
    {
        $response = $this->get(route('download.tree', ['id' => 1]).$request, $this->getHeaderRelativeSsoToken());
    ...
    {
        $response = $this->getJson(route('download.tree', ['id' => 1]), $this->getHeaderUserSsoToken());
