...
final class PersonStoredControllerTest extends TestCase
{
    ...
    public function test_store200(array $request, callable $expected): void
    {
        $response = $this->postJson(
            route('person.store'),
            $request,
            headers: $this->getHeaderAdminSsoToken()
        );
        $response->assertStatus(200);
        $expected($this, $response['person_id']);
        dump('⚠️  Проверь в базе данных test_danshin_notice создание записи в таблице genealogy_dates_persons - person_id - 23');
    }
    ...
    public function test_store403(): void
    {
        $response = $this->postJson(
            route('person.store'),
            headers: $this->getHeaderRelativeSsoToken()
        );
