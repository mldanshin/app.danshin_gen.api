...
final class DownloadPersonControllerTest extends TestCase
{
    ...
    public function test200(int $id, string $request): void
    {
        ...
        $response = $this->get(route('download.person', ['id' => $id]).$request, $this->getHeaderRelativeSsoToken());
    ...
    {
        $response = $this->get(route('download.person', ['id' => 1]).$request, $this->getHeaderRelativeSsoToken());
    ...
    {
        $response = $this->getJson(route('download.person', ['id' => 1]), $this->getHeaderUserSsoToken());
