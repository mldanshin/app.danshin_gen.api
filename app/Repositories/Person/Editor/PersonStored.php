...
final class PersonStored
{
    ...
    public function store(PersonModel $model): int
    {
        ...
        $deathDate = ($model->isLive === true) ? null : $model->deathDate?->string;

        $personId = DB::transaction(
            function () use ($eloquentModel, $model, $birthDate, $deathDate) {
                ...
                $eloquentModel->birth_date = $birthDate;
                $eloquentModel->birth_place = $model->birthPlace;
                $eloquentModel->death_date = $deathDate;
                ...
                if ($birthDate !== null && !str_contains($birthDate, '?')) {
                    if (!$this->noticeService->createPerson($eloquentModel->id, DateType::BIRTH)) {
                        throw new ServerException('Failed to create birth notice for person: ' . $eloquentModel->id);
                    }
                }

                if ($deathDate !== null && !str_contains($deathDate, '?')) {
                    if (!$this->noticeService->createPerson($eloquentModel->id, DateType::DEATH)) {
                        throw new ServerException('Failed to create death notice for person: ' . $eloquentModel->id);
                    }
                }
